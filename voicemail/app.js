...
app.post("/incoming-call", (req, res) => {
  ...
  twiml.say("Hello. Thank you for calling. Please join us as we say goodbye to our dear friend, the internet. We'll need some information from you. We'll ask you some questions. At the tone, say your answer. Once you are done answering, press 1.");
...
app.post("/ask-name", (req, res) => {
  ...
  twiml.record({
    ...
    recordingStatusCallback: `/recording-complete?question=name&callSid=${callSid}&callerId=${encodeURIComponent(callerId)}`
  });
...
app.post("/ask-hate", (req, res) => {
  ...
  twiml.record({
    ...
    recordingStatusCallback: `/recording-complete?question=hate&callSid=${callSid}&callerId=${encodeURIComponent(callerId)}`
  });
...
app.post("/ask-like", (req, res) => {
  ...
  twiml.record({
    ...
    recordingStatusCallback: `/recording-complete?question=like&callSid=${callSid}&callerId=${encodeURIComponent(callerId)}`
  });
...
app.post("/ask-memory", (req, res) => {
  ...
  twiml.record({
    ...
    recordingStatusCallback: `/recording-complete?question=memory&callSid=${callSid}&callerId=${encodeURIComponent(callerId)}`
  });
...
app.post("/ask-message", (req, res) => {
  ...
  twiml.say("What is something you wish you could say to the internet if they were still here?");
  twiml.record({
    maxLength: 120,
    playBeep: true,
    finishOnKey: "1",
    action: `/end-call?callerId=${encodeURIComponent(callerId)}`,
    recordingStatusCallback: `/recording-complete?question=message&callSid=${callSid}&callerId=${encodeURIComponent(callerId)}`
  });
...
app.post("/recording-complete", async (req, res) => {
  try {
    ...
    let callerId = req.query.callerId.replace(/[^a-zA-Z0-9]/g, "_").replace(/^_+/, "");
